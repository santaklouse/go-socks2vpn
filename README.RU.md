...
Android-приложение поддерживает IPv4 и IPv6 и работает как foreground VPN service. До установки full-tunnel маршрута оно выполняет через указанный прокси полноценный SOCKS handshake с авторизацией, поэтому доступный порт с неверными реквизитами теперь приводит к явной ошибке, а не к ложному состоянию Connected. DNS-запросы Android передаются через тот же SOCKS-прокси как RFC 8484 DNS-over-HTTPS по TCP на `1.1.1.1:443`: работа браузера больше не зависит от поддержки SOCKS5 `UDP ASSOCIATE`, а DNS работает и с SOCKS4. В GUI есть такая же лампа состояния, суммарный трафик сеанса и живые шкалы download/upload. Пароль живёт только в памяти процесса до отключения.
