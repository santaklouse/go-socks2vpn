...
Android-приложение поддерживает IPv4 и IPv6, задаёт DNS `1.1.1.1`, выполняет TCP preflight прокси и работает как foreground VPN service. В GUI есть такая же лампа состояния, суммарный трафик сеанса и живые шкалы download/upload. Пароль живёт только в памяти процесса до отключения.
